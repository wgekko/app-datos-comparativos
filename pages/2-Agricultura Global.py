...
@st.cache_data
def load_data():
    df = pd.read_excel("datos/agricultura-cereales-ganaderia-alimentos.xlsx")
    return df

df = load_data()

st.header(":material/agriculture: Análisis de Agricultura a Nivel Global")
st.write("---")

st.header(":material/agriculture: Gráficos de Dispersión mejorados con gráficos de Margen y animaciones")
...
variables = ['Consumo Fertilizante x has',	'Indice Prod. Ganadera % PBI',	'Inidice Prod. Alimentos % PBI', 
            'Rendimiento x Cereales x Has', 'Tierra produccion de cereales x Has']
